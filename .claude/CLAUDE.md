...
## 작업 표면 라우팅
- `mobile/**` 작업은 아래 Expo/RN 규칙을 적용하고 `mobile/AGENTS.md`의 버전 문서를 먼저 읽는다.
- `client/**` 웹 작업은 Vite(5173) + Express API(3000)다. 웹 팔로우/로그인은 `docs/workflow/web-follow-login-workflow.md`와 `automation/web-follow-login/` 계약을 우선 적용한다.
- 웹 작업에 Expo 딥링크/Metro 검증을 적용하지 않는다. 모바일 작업에 Vite 브라우저 검증을 적용하지 않는다.
...
## 웹 팔로우/로그인 루프
- 시작과 종료에 `pnpm harness:web-follow status`를 실행한다. `NEXT_PHASE`의 Phase 하나만 처리한다.
- 최신 웹 화면은 `http://localhost:5173`, `/api`는 Express `http://localhost:3000`이다.
- OAuth 앱 복귀 URL은 현재 `window.location.origin`을 사용하며 localhost:5173과 발표 배포 origin을 Supabase allowlist에 등록한다.
- live DB write, `.env`, dependency install, OAuth dashboard, git publish는 사람 승인 없이는 실행하지 않는다.
