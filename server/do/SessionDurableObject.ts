...
export class SessionDurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (method === "GET" && url.pathname === "/info") {
      const members: SessionMemberState[] =
        (await this.state.storage.get("members")) ?? [];
      const swipes: SwipeState[] =
        (await this.state.storage.get("swipes")) ?? [];
      const status: string =
        (await this.state.storage.get("status")) ?? "WAITING";

      return new Response(JSON.stringify({ members, swipes, status }), {
    ...
    if (method === "POST" && url.pathname === "/join") {
      ...
      return new Response(
        JSON.stringify({ success: true, count: members.length }),
        {
          headers: { "Content-Type": "application/json" },
        },
      );
    ...
    if (method === "POST" && url.pathname === "/swipe") {
      ...
      return new Response(
        JSON.stringify({ success: true, count: swipes.length }),
        {
          headers: { "Content-Type": "application/json" },
        },
      );
    ...
    if (method === "GET" && url.pathname === "/result") {
      ...
      return new Response(
        JSON.stringify({ success: true, N, lambda, results }),
        {
          headers: { "Content-Type": "application/json" },
        },
      );
