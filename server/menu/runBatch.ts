...
// 결과:  server/data/menus.json  ({ id, url, ok, format, items }[])
//
...
async function main() {
  ...
  console.log(`대상 ${targets.length}곳 (website 보유) · ${dryRun ? "DRY-RUN (키 없음)" : "실제 추출"} · 모델 ${process.env.MENU_MODEL || "haiku"}\n`);

  const out: unknown[] = [];
  let okFetch = 0, withMenu = 0;
  for (let i = 0; i < targets.length; i++) {
    const r = targets[i];
    const res = await extractMenu(r.website!);
    if (res.ok) okFetch++;
    if (res.items.length) withMenu++;
    out.push({ id: r.id, name: r.name, url: r.website, ok: res.ok, format: res.format, error: res.error, items: res.items });
    console.log(`  [${i + 1}/${targets.length}] ${res.ok ? "✅" : "❌"} ${res.format ?? "-"}\t${r.name}  ${res.error ? "(" + res.error + ")" : res.items.length ? res.items.length + "개 메뉴" : ""}`);
    if (i < targets.length - 1) await sleep(DELAY_MS);
  }

  const outPath = join(dir, "menus.json");
  writeFileSync(outPath, JSON.stringify(out, null, 2));
  console.log(`\n결과: fetch 성공 ${okFetch}/${targets.length} · 메뉴 추출 ${withMenu} → ${outPath}`);
  if (dryRun) console.log("※ DRY-RUN — LLM 미호출. ANTHROPIC_API_KEY 설정 후 재실행하면 실제 menu_items가 채워집니다.");
}
