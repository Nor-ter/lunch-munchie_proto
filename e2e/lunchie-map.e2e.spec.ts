...
test('saved restaurant uses its configured map and returns to the unified saved list', async ({ page }) => {
  ...
  await expect(page.getByRole('group', { name: '저장 항목 필터' })).toBeVisible();
