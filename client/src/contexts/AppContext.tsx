...
export async function buildDeck(
  ...
  try {
    ...
    if (!res.ok) return { restaurants: base, dietaryBestEffort };
    ...
    return { restaurants: slate.length ? slate : base, slateId: data.slate_id, recMeta: meta, modelVersion: data.model_version, dietaryBestEffort };
  } catch {
    return { restaurants: base, dietaryBestEffort };
  }
}
...
export function AppProvider({
  ...
  const fetchSession = useCallback(async (token: string, catalogueOverride?: Restaurant[]): Promise<GroupSession> => {
    ...
    const deck = !isActiveQuickMatchStatus(status)
      ...
      : sharedRestaurants.length === sharedDeckIds.length && sharedRestaurants.length > 0
      ? {
