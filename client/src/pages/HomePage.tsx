...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    try {
      const legacy = localStorage.getItem('lm_today_journey') ?? '[]';
      localStops = JSON.parse(localStorage.getItem('lm_lunchie_journey') ?? legacy) as JourneyStop[];
            ...
            >
              ...
              <section className="mt-5 rounded-[22px] border border-[#F1D8CC] bg-[#FFF5EF] p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-black text-[#DB6C66]">{todayJourneyStops.length}곳</span>
                </div>
                {todayJourneyStops.length > 0 ? (
                  <div className="mt-3 space-y-2">
                    {todayJourneyStops.map((stop, index) => (
                      <div key={`${stop.restaurant_id}-${stop.at}`} className="flex items-center gap-2.5 rounded-2xl bg-white px-3 py-2.5">
