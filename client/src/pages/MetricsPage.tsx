...
interface Mechanism {
  ...
  chainTransitions: { from: string; to: string; count: number; p: number }[];
}
interface Metrics {
  total: number;
  ...
  mechanism: Mechanism;
  engine: { modelVersion: string; dim: number; users: number; learnedUsers: number; avgThetaNorm: number | null; exposureTracked: number; satiationUsers: number; satiationCats: number; chainTransitions: number; chainObservations: number; explicitSignals: number };
    ...
    <div className="min-h-dvh bg-white p-5 max-w-[820px] mx-auto pb-24">
      ...
      <section className="mb-6">
        ...
        <div className="mb-3 p-2.5 rounded-lg bg-[#EAF7EC] text-[11px] text-[#2E6B36]">
          <b>엔진 가동</b> — <code>{m.engine.modelVersion}</code> · Thompson 탐색(불확실성 기반) · 취향 학습 {m.engine.learnedUsers}/{m.engine.users}명 · 노출피로 {m.engine.exposureTracked}쌍 · satiation {m.engine.satiationCats}쌍 · 연쇄 {m.engine.chainObservations}건 · 명시신호 {m.engine.explicitSignals}건(저장·우승 가중) · score = 평판+맥락+취향−노출피로±재소비+연쇄
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* A. 노출 피로 곡선 */}
          <MCard title="노출 피로 — 누적 노출 ↔ LIKE율" hint="v1 패널티 가동 — 재노출 카드는 점수↓로 덜 노출/덜 수락">
          ...
          {/* A3. 음식 연쇄 (Munchie 다음-스톱) */}
          <MCard title="음식 연쇄 (Munchie) — 학습된 다음-스톱" hint="직전 스톱 다음에 뭐가 따라오나 P(next|prev). NEXT_STOP 추천 근거">
            {m.mechanism.chainTransitions.length > 0 ? (
              m.mechanism.chainTransitions.map((t) => (
                <div key={t.from + '>' + t.to} className="flex items-center justify-between text-[11px] mb-1">
                  <span className="text-[#1A1A1A]"><b>{t.from}</b> → {t.to}</span>
                  <span className="tabular-nums text-[#6E6E6E]">{Math.round(t.p * 100)}% <span className="text-[#9A9A9A]">·{t.count}건</span></span>
                </div>
              ))
            ) : (
              <div className="text-[11px] text-[#9A9A9A]">아직 연쇄 데이터 없음 (occasion 시퀀스 필요)</div>
            )}
          </MCard>
          {/* B. 분별력 (score → 선호 예측) */}
