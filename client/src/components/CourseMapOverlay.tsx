...
export default function CourseMapOverlay({ course }: { course: Course }) {
  ...
  if (stops.length === 1) {
    ...
    const lats = stops.map(s => s.lat ?? 37.5);
    const lngs = stops.map(s => s.lng ?? 127.0);
    ...
    pts = stops.map(s => ({
      x: 15 + ((s.lng ?? 127.0) - minLng) / lngRange * 70,
      y: 15 + (1 - ((s.lat ?? 37.5) - minLat) / latRange) * 70,
    }));
  }
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      {pts.map((pt, i) => (
        <div
          key={i}
          className="absolute w-5 h-5 bg-[#EB5053] rounded-full border-2 border-white flex items-center justify-center text-white font-bold text-[11px] shadow-md"
