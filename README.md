...
### 2026-08-07 Lunchie·Munchie 통합 UX 업데이트

#### Quick Match 설정과 세션 시작

- 마감 시간을 1~15분 범위에서 설정하는 대형 원형 타이머 UI로 개편
  - 원형 다이얼 드래그, 키보드 방향키, 중앙 숫자 직접 입력, 위·아래 증감 버튼 지원
  - 예상 종료 시각과 5분·10분·15분 빠른 선택 제공
  - 원 밖의 중복 입력창과 안내 문구 제거
- 초대받은 참여자에게 호스트용 QR·방 관리 UI를 노출하지 않고 참여 완료 화면만 제공
- 호스트가 세션을 시작하면 모든 참여자가 별도의 준비 동작 없이 같은 예선전으로 자동 이동
- 세션 상태를 1초 간격으로 동기화하여 호스트와 참여자의 시작 타이밍을 일치시킴
...
- 그룹 결승전도 솔로 모드와 동일한 사선 대결 구도로 통일
- 그룹 대기·결과·공유 화면의 로고를 사용자 장비가 반영된 런치킨 캐릭터로 교체
- 결과 카드의 음식점 이미지를 원형 영역에 안정적으로 렌더링하고 공유 카드에도 같은 런치킨 브랜드 적용

#### 런치 투표 대기 도우미

- 예선 투표를 끝낸 사용자가 기다리는 동안 Munchie Feed를 둘러볼 수 있는 런치킨 플로팅 도우미 추가
- 활성 Lunchie 세션에서 대기 중일 때만 전역 표시하며 다른 세션이나 일반 Munchie 이용 중에는 숨김
- 남은 투표 시간을 말풍선으로 안내하고 결승·재추천 단계가 열리면 투표 페이지 복귀 동작 제공
- 런치킨 터치 시 표정·점프·말풍선이 변하는 간단한 상호작용 지원
- 결과 API는 읽기 전용으로 조회하여 기존 세션·투표 데이터를 변경하지 않음

#### Munchie 스토리 공유

- 먼치피드 게시물의 정확한 ID를 공유 화면에 전달하여 동일 코스의 다른 게시물이 선택되는 문제 방지
- 기존 피드의 템플릿, 사진 앨범, 배치, 드로잉을 수정 없이 그대로 사용하는 읽기 전용 공유 미리보기 제공
- `피드`와 `맵` 중 Instagram 스토리에 담을 화면을 선택할 수 있으며 두 화면 모두 정확한 9:16 비율로 렌더링
- 공유 화면의 사진 업로드·교체·삭제·회전·드래그 기능을 제거하고 원본 피드를 고정
- 모바일 공유 시 1080px PNG를 생성하며 피드 링크 복사와 기기 저장 지원

#### Munchie Feed 템플릿 통합

- 기존 9:16 스토리 디자인 19개를 원본 픽셀을 보존한 540×720 피드 자산으로 변환
- 기존 6개를 포함한 총 25개 템플릿을 코스 생성·피드 수정 팔레트에 동일하게 제공
- 디자인 유형별 사진 슬롯을 연결하고 인쇄된 사진 영역을 투명 프레임으로 처리해 실제 앨범 사진 렌더링 지원
- `/templates`의 별도 `스토리 공유 템플릿` 섹션을 제거하고 모든 디자인을 `맛집 피드 템플릿`으로 통합

#### 브라우저 표시와 로컬 실행

- 모바일 브라우저의 자동 다크모드 재색상을 차단하고 모든 화면과 공유 결과를 라이트 모드로 고정
- 로컬 Pages 개발 서버가 `0.0.0.0`에서 수신하도록 설정하여 같은 네트워크의 휴대폰 테스트 지원
...
- 사진 없는 기본 상태로 확인하는 4:3 맛집 피드 템플릿 브라우저
- 기존 피드 템플릿 6개와 스토리 디자인에서 변환한 19개를 합친 총 25개 디자인 탐색
- 게시 완료 화면의 `Munchie 홈으로`와 `공유하기` 버튼을 통한 공유 흐름
- 피드 게시물별 공유 링크를 통해 원본 사진 앨범·배치·드로잉을 정확히 복원
- 공유 단계에서는 사진 변경을 금지하고 `피드` 또는 `맵` 9:16 미리보기만 선택
- 최종 9:16 결과를 1080px PNG로 저장하거나 Instagram 스토리 공유 시트로 전달
- 해당 Munchie 피드 링크 복사 지원
