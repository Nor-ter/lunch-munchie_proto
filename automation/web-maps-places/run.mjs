// web-maps-places 하네스 — automation/web-follow-login/run.mjs 미러.
// 차이: Phase 0 structural 체크를 manifest.phases[].checks(데이터 주도)에서 읽어 재사용성 확보.
...
const HARNESS = "web-maps-places";
const root = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const manifest = JSON.parse(readFileSync(resolve(root, `automation/${HARNESS}/manifest.json`), "utf8"));
const artifactDir = resolve(root, `.artifacts/${HARNESS}`);
...
function structural(phase) {
  ...
  if (checks) {
    if (checks.deps) {
      ...
      for (const dep of checks.deps) {
        if (!pkg.dependencies?.[dep] && !pkg.devDependencies?.[dep]) missing.push(`dependency:${dep}`);
      }
    }
    if (checks.envKeys) {
      const example = existsSync(resolve(root, ".env.example")) ? readFileSync(resolve(root, ".env.example"), "utf8") : "";
      const envPath = resolve(root, ".env");
      const env = existsSync(envPath) ? readFileSync(envPath, "utf8") : "";
      for (const key of checks.envKeys) {
